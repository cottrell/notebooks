...
download() {
    ...
    if has_cmd curl; then
        curl -fsSL -H "User-Agent: Mozilla/5.0" "$url" -o "$dest"
...
try_download() {
    ...
    if has_cmd curl; then
        curl -fsSL -H "User-Agent: Mozilla/5.0" "$url" -o "$dest" 2>/dev/null
    elif has_cmd wget; then
        wget -q --user-agent="Mozilla/5.0" "$url" -O "$dest" 2>/dev/null
    else
        return 1
    fi
}
...
get_latest_version() {
    ...
    if has_cmd curl; then
        version=$(curl -fsSL -H "User-Agent: Mozilla/5.0" "$url" | grep '"tag_name"' | sed -E 's/.*"tag_name": *"v?([^"]+)".*/\1/')
    elif has_cmd wget; then
        version=$(wget -q --user-agent="Mozilla/5.0" -O- "$url" | grep '"tag_name"' | sed -E 's/.*"tag_name": *"v?([^"]+)".*/\1/')
    else
        error "Neither curl nor wget found. Please install one of them."
    fi
